...
The :class:`~vllm_metal.v1.mtp_heads.NativeMTPHead` that registers this head
imports its model and loader lazily, so ``import vllm_metal.v1.mtp_heads`` stays
free of mlx / mlx_lm.
"""
            ...
            for e, key in enumerate(expert_keys):
                if key not in weights:
                    raise ValueError(
                        ...
                        f"switch_mlp. {_HEAD_SOURCE_HINT}"
                    )
...
class Glm4MoeLiteMTPModel(nn.Module):
    ...
    def forward_slots(
        ...
        *,
        ...
        drafting at the wrong position. Omitted by default so existing callers
        stay unaffected.
        """
        if x.ndim != 2:
            raise ValueError(
    ...
    def from_config(cls, config: Mapping[str, Any]) -> Glm4MoeLiteMTPHeadMetadata:
        ...
        if model_type != GLM4_MOE_LITE_MTP_MODEL_TYPE:
            raise ValueError(
                "Glm4MoeLiteMTP head requires "
                f"model_type={GLM4_MOE_LITE_MTP_MODEL_TYPE!r}, got "
                f"{model_type!r}. {_HEAD_SOURCE_HINT}"
            )
        ...
        if not isinstance(text, Mapping):
            raise ValueError(
                ...
                f"under 'text_config' (the drafter-split schema). {_HEAD_SOURCE_HINT}"
            )
    ...
    def _preflight_config(
        ...
        if head_config is None and self._load_model is None:
            raise ValueError(
                ...
                f"{model_path}. {_HEAD_SOURCE_HINT}"
            )
