...
Structure and the slab rewind/hole/accepted-rows-only rules mirror the DFlash
proposer (``vllm_metal.v1.dflash_proposer``); unlike DFlash we reuse the runner's
existing ``target_hidden_states`` (no aux-hidden-state machinery) and the KV slab
holds the head layer's own MLA latent (different k/v feature dims) rather than
re-projected context K/V.
"""
...
class NativeMTPProposer:
    ...
    def __init__(
        ...
        *,
        ...
        # Retained for parity with the other proposers' build signature; the slab
        # infers its k/v dtype from the first append, so it is not needed to size
        # the slabs.
        self._dtype = dtype
    ...
    def build(
        ...
        *,
        ...
        return cls(model=runtime.model, controller=controller, dtype=dtype)
