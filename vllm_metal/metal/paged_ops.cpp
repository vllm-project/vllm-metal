...
void paged_attention_v2_online_impl_common(
  ...
  int max_num_partitions =
      std::max(1, (max_seq_len + PARTITION_SIZE - 1) / PARTITION_SIZE);
  bool use_partitioning =
      PARTITION_SIZE % block_size == 0 && max_num_partitions > 1;
  ...
  std::string kname =
      ...
      "_bs" + std::to_string(block_size) +
      "_nt256_nsl32_ps" +
      std::to_string(use_partitioning ? PARTITION_SIZE : 0);
  ...
  const int32_t grid_z = static_cast<int32_t>(max_num_partitions);
  enc.dispatch_threadgroups(
