...
def _build_v3_paged_attention_source() -> str:
    """Concatenate float8 + utils + attn_common + turboquant + v3 kernels.

    Order matters: attn_common.h provides Vec<float/bfloat/half/uchar>
    specializations, FP8 helpers, Qk_dot, find_seq_idx, resolve_seq_and_q_block,
    and function constants. turboquant.metal extends with Vec<char>, is_char,
    and TQ-specific helpers. The two kernel files (vector and MMA) contain
    kernel definitions and instantiations for their respective compute paths;
    they share the same MLX library slot.
    """
    parts = [
        f"#define VLLM_METAL_PARTITION_SIZE {PARTITION_SIZE}",
        ...
        _read_metal_source(_KERNELS_V3_DIR / "paged_attention_vector.metal"),
        _read_metal_source(_KERNELS_V3_DIR / "paged_attention_mma.metal"),
    ]
    return "\n".join(parts)
