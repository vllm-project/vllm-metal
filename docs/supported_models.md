...
| DeepSeek-R1-Distill-Llama-8B | ✅ | GQA (paged) | ✅ | [#407](https://github.com/vllm-project/vllm-metal/pull/407) | LlamaForCausalLM architecture, reuses the Llama 3 GQA paged path (no code changes). Validated on Mac Studio (Apple M3 Ultra, 96 GB) on macOS 26.3.1 with `mlx-community/DeepSeek-R1-Distill-Llama-8B-4bit`; greedy output matches `mlx_lm` reference (4/4 prompts exact through 16 tokens on the paged kernel) |
