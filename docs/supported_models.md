...
| DeepSeek-V2-Lite | ✅ | MLA (paged latent cache, MLX SDPA default; experimental Metal single-pass decode kernel opt-in via `VLLM_METAL_MLA_KERNEL=1`) | 🟡 | RFC [#360](https://github.com/vllm-project/vllm-metal/issues/360) | Validated with `mlx-community/DeepSeek-Coder-V2-Lite-Instruct-4bit` on MacBook Pro (Apple M5 Max, 36 GB): greedy decode token-for-token match between MLX SDPA and Metal kernel paths |
