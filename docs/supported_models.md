...
| Granite 3.3 | 🔵 | GQA (paged) | ✅ | [#403](https://github.com/vllm-project/vllm-metal/pull/403) | Validated on MacBook Pro (Apple M4 Pro, 48 GB) with `mlx-community/granite-3.3-8b-instruct-4bit`; greedy output matches the independent `mlx_lm` reference byte-for-byte (see `tests/test_granite_golden.py`). Default-on for non-hybrid paged models. Granite's four scalar multipliers (embedding/residual/attention/logits) are applied inside mlx-lm's Granite model forward (`mlx_lm.models.granite`), and `attention_multiplier` reaches the Metal kernel via `inner.scale`, so no Metal adapter is needed |
