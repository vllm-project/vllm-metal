...
| `VLLM_METAL_DSPARK_PAGED_CONTEXT` | `0` | Keep the drafter's context K/V in a scheduler-owned KV-cache group in the target's own block layout, registered like the draft model's (`draft_layers.*`, one `FullAttentionSpec` per drafter layer) and read by the same paged-attention kernel the target uses. The scheduler sizes, allocates, hashes and evicts the committed context; only the draft block's scratch positions stay proposer-local. Nothing is reserved on the drafter's side for the context or for a whole-buffer rewrite, which returns that memory to the target's KV cache. Requires `--no-enable-prefix-caching`: a target prefix-cache hit skips the target forward, so no hidden states exist to fill the drafter's group for those positions. Requires a drafter head dimension the kernels are built for (64, 80, 96, 112, 128, 192, 256, 512); any other keeps the arena and logs a warning. |
