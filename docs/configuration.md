...
| `VLLM_METAL_TOOLSPEC_CAPACITY` | `512` | Retrieval-augmented drafting only (`vllm_metal.v1.toolspec_proposer.ToolSpecProposer`; see [Speculative decoding](speculative_decoding.md#retrieval-augmented-drafting-toolspec)). How many finished invocations the per-worker memory holds before evicting the oldest. Each entry is one question vector (hidden width × 4 bytes) plus a truncated token trace. Ignored by every other speculative method. |
| `VLLM_METAL_TOOLSPEC_TOP_K` | `4` | How many similar past invocations to n-gram search per drafting step. Larger values search more traces for a match at proportional CPU cost. |
| `VLLM_METAL_TOOLSPEC_NGRAM_MAX` | `7` | Longest suffix needle tried when matching against retrieved traces. |
| `VLLM_METAL_TOOLSPEC_NGRAM_MIN` | `5` | Shortest suffix needle tried. Below 5 tokens a needle matches almost anything in JSON-shaped text and mostly produces drafts the target rejects. |
