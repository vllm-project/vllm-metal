...
| `VLLM_METAL_NATIVE_SAMPLING` | `0` | Opt-in MLX-native non-greedy sampling with temperature/top-k/top-p/min-p for eligible pure-decode batches. Requests in a native batch must share top-k, top-p, and min-p settings; seeds, penalties, logprobs, or token constraints keep the torch path. `min_p` is also supported by the default torch sampler; `logit_bias` remains unsupported. Sampled tokens come from the MLX RNG keyed off the engine seed, so sampled sequences can differ from torch's. Set to `1` to enable. |
