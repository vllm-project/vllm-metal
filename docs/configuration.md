...
| `VLLM_METAL_MLA_SPLIT_KV` | `0` | Enable the experimental long-context split/reduce subpath inside `VLLM_METAL_MLA_KERNEL=1`. Off by default while absorbed-model benchmarks are still negative. Set to `1` only when testing the split kernel directly. |
