...
class Platform:
    ...
    def is_device_capability_family(
        ...
        if isinstance(current_capability, tuple):
            # Mac 可能返回 (major, minor) 元组，需手动转 int
            current_int = current_capability[0] * 10 + current_capability[1]
        else:
            # 正常情况，调用 .to_int()
            current_int = current_capability.to_int()
        
        return (current_int // 10) == (capability // 10)
        # ===========================
