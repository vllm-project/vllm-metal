...
hand-built ``ProposeContext`` objects drive the proposer.
"""
...
class TestNativeMTPProposerIngest:
    ...
    def test_prune_on_finish_removes_slab_and_pending(self) -> None:
        ...
        # Next step: the scheduler reports r1 finished -> its state is dropped.
        assert (
            proposer.propose(_ctx(hidden=_hidden(1), finished_req_ids=["r1"])) is None
        )
        assert "r1" not in proposer._slabs
        assert "r1" not in proposer._pending_last_hidden

    def test_finished_id_reused_in_the_same_step_rebuilds_from_zero(self) -> None:
        proposer, _ = _proposer()
        first = [5, 6, 7]
        proposer.propose(
            _ctx(
                hidden=_hidden(len(first)),
                prefill_reqs=[_prefill("r1", first)],
                prefill_token_ids=[0],
                prefill_result_modes=["intermediate"],
                request_states={"r1": _request_state(first)},
                cu_seqlens=[0, len(first)],
            )
        )

        # vLLM can hand a finished id straight back to a new request in the same
        # step, so the reused id must restart rather than continue the old slab.
        reused = [11, 12]
        reuse_ctx = {
            "prefill_reqs": [_prefill("r1", reused)],
            "prefill_token_ids": [0],
            "prefill_result_modes": ["intermediate"],
            "request_states": {"r1": _request_state(reused)},
            "cu_seqlens": [0, len(reused)],
        }
        proposer.propose(
            _ctx(hidden=_hidden(len(reused)), finished_req_ids=["r1"], **reuse_ctx)
        )

        fresh, _ = _proposer()
        fresh.propose(_ctx(hidden=_hidden(len(reused)), **reuse_ctx))
        assert proposer._slabs["r1"].offset == fresh._slabs["r1"].offset
