...
def test_unsupported_contract_and_bypassed_capture_fail_explicitly():
    unknown = SimpleNamespace(model=SimpleNamespace(layers=[lambda x: x]))
    with pytest.raises(NotImplementedError, match="capture is not implemented"):
        AuxHiddenStateCapture(unknown, (1,))
    model = _model("llama")
    capture = AuxHiddenStateCapture(model, (1, 3, 5))
    with pytest.raises(RuntimeError, match="bypassed auxiliary capture"):
        capture.run(lambda: mx.zeros((1, 1, 128)))
    native = mx.compile(model)
    tokens = mx.array([[1, 2]])
    mx.eval(native(tokens))
    with pytest.raises(RuntimeError, match="bypassed auxiliary capture"):
        capture.run(native, tokens)
