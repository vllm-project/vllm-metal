...
def test_from_config_parses_valid_head() -> None:
    ...
    assert metadata.architectures == ("Glm4MoeLiteMTPModel",)
    assert metadata.hidden_size == 2048
    assert metadata.vocab_size == 154880
    assert metadata.kv_lora_rank == 512
    assert metadata.num_hidden_layers == 0
    assert metadata.num_nextn_predict_layers == 1


def test_from_config_defaults_num_nextn_when_absent() -> None:
    config = _head_config()
    del config["num_nextn_predict_layers"]

    metadata = Glm4MoeLiteMTPHeadMetadata.from_config(config)
...
def test_from_config_rejects_nonzero_num_hidden_layers() -> None:
    with pytest.raises(ValueError, match="num_hidden_layers=0") as excinfo:
        Glm4MoeLiteMTPHeadMetadata.from_config(_head_config(num_hidden_layers=47))
    assert EXTRACTION_TOOL in str(excinfo.value)
...
def test_from_config_rejects_bad_n_predict() -> None:
    with pytest.raises(ValueError, match="n_predict=1"):
        Glm4MoeLiteMTPHeadMetadata.from_config(_head_config(n_predict=3))
...
@pytest.mark.parametrize("field", list(_COMPAT_FIELDS))
def test_compat_fields_are_all_checked(field: str) -> None:
    # Guards against silently dropping a field from the compat set.
    metadata = Glm4MoeLiteMTPHeadMetadata.from_config(_head_config())
...
def test_get_model_classes_rejects_wrong_model_type() -> None:
    with pytest.raises(ValueError, match=EXTRACTION_TOOL):
...
def test_loader_rejects_full_target_repo_via_config(tmp_path: Path) -> None:
    (tmp_path / "config.json").write_text(
        json.dumps(_head_config(num_hidden_layers=47))
    )
    loader = Glm4MoeLiteMTPHeadLoader(
        load_model_fn=lambda *a, **k: pytest.fail("load_model called"),
        download_fn=lambda name, rev: tmp_path,
    ...
    with pytest.raises(ValueError, match="num_hidden_layers=0"):
        loader.load_if_needed(
            speculative_config=_speculative_config(),
            target_config=_target_config(),
        )
