...
from tests.test_shortconv_hybrid_spec import make_lfm2_args
...
def _make_wrapper(
    ...
    wrapper = ShortConvPagedWrapper(module, cache_idx=0, state_cache=state_cache)
    return wrapper, state_cache


def _set_packed_context(seg_lens: list[int], slot_ids: list[int]) -> None:
    cu_seqlens = [0]
    ...
    set_context(
        PagedAttentionContext(
            ...
            gdn_slot_mapping=list(slot_ids),
...
def _run_wrapper_step(
    ...
    _set_packed_context(seg_lens, slot_ids)
...
class TestShortConvPagedWrapper:
    def test_packed_multi_request_matches_sequential_reference(self) -> None:
