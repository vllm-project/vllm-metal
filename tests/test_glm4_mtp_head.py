...
def test_build_proposer_rejects_pipeline_parallelism() -> None:
    with pytest.raises(NotImplementedError, match="pipeline parallel"):
        Glm4MoeLiteMTPHead().build_proposer(_context(pipeline_parallel_size=2))
...
def test_build_proposer_delegates_to_native_mtp_proposer(
    ...
    # A fake head returns a proposer-shaped object, not a bare sentinel.
    class _FakeProposer:
        def needs_target_hidden_states(self, *args: object, **kwargs: object) -> bool:
            return True

        def propose(self, ctx: object) -> None:
            return None
    ...
    def _fake_build(**kwargs: object) -> _FakeProposer:
        captured.update(kwargs)
        return proposer

    monkeypatch.setattr(mtp_proposer.NativeMTPProposer, "build", _fake_build)
    context = _context(target_config={"hidden_size": 2048})
