...
def test_spec_decode_metrics_computes_acceptance_rate() -> None:
    ...
    metrics = bench.spec_decode_metrics(_Llm(), require=True)
...
def test_spec_decode_metrics_requires_counters_for_mtp_run() -> None:
    class _Llm:
        def get_metrics(self):
            return [SimpleNamespace(name="vllm:other_metric", value=1)]

    # An MTP run whose engine reports no draft/accepted counters fails loud
    # rather than emitting a result with the promised acceptance evidence missing.
    with pytest.raises(RuntimeError, match="spec_decode_num_draft_tokens"):
        bench.spec_decode_metrics(_Llm(), require=True)


def test_spec_decode_metrics_baseline_allows_missing_counters() -> None:
    class _Llm:
        def get_metrics(self):
            return [SimpleNamespace(name="vllm:other_metric", value=1)]

    metrics = bench.spec_decode_metrics(_Llm(), require=False)
